...
## Тепловая карта изначальных численных признаков
...
XGBoost — это высокоэффективная реализация градиентного бустинга. Алгоритм строит ансамбль деревьев последовательно, используя регуляризацию для контроля сложности модели и защиты от переобучения, а также эффективные параллельные вычисления и оптимизированную работу с разреженными матрицами.
...
Многослойный перцептрон представляет собой полносвязную нейронную сеть с входным, несколькими скрытыми слоями с нелинейной функцией активации и выходным линейным слоем. Сеть способна моделировать сколь угодно сложные нелинейные зависимости, обучаясь методом обратного распространения ошибки.
